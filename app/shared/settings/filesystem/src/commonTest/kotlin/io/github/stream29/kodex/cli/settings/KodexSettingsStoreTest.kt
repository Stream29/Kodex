...
import io.github.stream29.kodex.openai.codexclistorage.CodexCliHookDeclarations
import io.github.stream29.kodex.openai.codexclistorage.CodexCliHookHandler
import io.github.stream29.kodex.openai.codexclistorage.CodexCliHookLayer
import io.github.stream29.kodex.openai.codexclistorage.CodexCliHookMatcher
import io.github.stream29.kodex.openai.codexclistorage.CodexCliHookMatcherGroup
...
val kodexSettingsStoreTest by testSuite(
    ...
    test("missing config and override files preserve compiled defaults") {
        withSettingsDirectory("missing") { root ->
            ...
            val defaults = KodexGlobalSettings(codexHome = root)
    ...
    test("reloads settings inherited from Codex Home and the project") {
        withSettingsDirectory("hook-refresh") { root ->
            ...
            val store: KodexGlobalSettingsStore = CodexCliStorage(root).openGlobalSettings(
                settingsDirectory = Path(root, "kodex"),
                workingDirectory = workingDirectory,
                defaults = KodexGlobalSettings(codexHome = root),
            )
    ...
    test("persists a complete Hook override instead of following later Codex changes") {
        withSettingsDirectory("hook-override") { root ->
            val kodexHooks = Path(root, "hooks.json")
            SystemCoroutineFileSystem.writeString(kodexHooks, """{"hooks":{"Stop":[]}}""")
            val store = CodexCliStorage(root).openGlobalSettings(root)
            val override = HookConfiguration(
                featureEnabled = false,
                sources = listOf(
                    CodexCliHookLayer(
                        sourcePath = Path(root, "kodex/custom-hooks.json"),
                        sourceKind = CodexCliHookSourceKind.Session,
                        environment = mapOf("KODEX_HOOK" to "override"),
            ...
            SystemCoroutineFileSystem.writeString(kodexHooks, """{"hooks":{"Stop":[{"matcher":"changed"}]}}""")
            store.reload()
    ...
    test("persists the selected authentication source") {
        withSettingsDirectory("auth-source") { root ->
            ...
            val updated = store.update { current ->
                current.copy(authSource = KodexAuthSource.Kodex)
            }

            assertEquals(KodexAuthSource.Kodex, updated.authSource)
            assertEquals(KodexAuthSource.Kodex, store.settings.value.authSource)
            assertEquals(
    ...
    test("persists a selected Codex Home separately from Codex configuration") {
        withSettingsDirectory("selected-home") { root ->
            ...
            val privateSettings = Path(root, "kodex")
            ...
            val store = storage.openGlobalSettings(
                ...
                defaults = KodexGlobalSettings(codexHome = initialHome),
            )
...
private fun globalSettingsPath(codexHome: Path): Path =
    Path(Path(codexHome, "kodex"), "settings.yml")
...
private suspend fun CodexCliStorage.openGlobalSettings(
    codexHome: Path,
    defaults: KodexGlobalSettings = KodexGlobalSettings(codexHome = codexHome),
): KodexSettingsStore =
    openGlobalSettings(
        settingsDirectory = Path(codexHome, "kodex"),
