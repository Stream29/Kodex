...
internal fun BoxScope.HookImportDialog(
    ...
    val filter = rememberHookInput(preview?.filter.orEmpty())
    var decisions by remember(preview?.id) {
        mutableStateOf<Map<String, HookImportDecision>>(emptyMap())
    }
    ...
    TuiDialog(
        ...
        Column(modifier = Modifier.fillMaxWidth().background(SettingsHomeBackground)) {
            ...
            HookInputField("Filter by source name or path", filter, width, autoFocus = true)
            Row {
                TuiButton(
                    label = "Preview",
                    color = SettingsForeground,
            ...
            preview?.let { current ->
                if (current.items.isEmpty()) {
                    Text("No matching Codex Hook sources.", color = SettingsForeground)
                }
                current.items.forEach { item ->
                    val decision = decisions[item.sourceKey] ?: HookImportDecision.Skip
                    Row(modifier = Modifier.fillMaxWidth()) {
                        Text(
                            value = "${item.displayName}: ${item.previewLabel()}",
                            color = SettingsForeground,
                        )
                        Text(" ")
                        TuiButton(
                ...
                TuiButton(
                    ...
                    color = SettingsForeground,
                    enabled = decisions.values.any { decision ->
...
private fun HookImportItem.previewLabel(): String =
    buildString {
        append(
            when (disposition) {
                HookImportDisposition.New -> "New"
                HookImportDisposition.Conflict -> "Conflict"
                null -> "Unsupported"
            },
        )
...
private fun HookImportDecision.previewLabel(): String =
    when (this) {
        HookImportDecision.Skip -> "Skip"
        HookImportDecision.Import -> "Import"
        HookImportDecision.Replace -> "Replace"
    }
