...
class TerminalTitleTest {
    @Test
    fun summaryCountsOnlyOpenedPersistedSessionTabs() {
        val summary = summarizeOpenSessions(
            listOf(
                SessionTabRenderState(
                    target = SessionTabTarget.OpenSession(sessionIndex = 1),
                    selected = false,
                    sessionName = "First",
                    running = true,
                ),
                SessionTabRenderState(
                    target = SessionTabTarget.NewSession(id = 2, ordinal = 1),
                    selected = true,
                    sessionName = "Draft",
                    running = true,
                ),
                SessionTabRenderState(
                    target = SessionTabTarget.OpenSession(sessionIndex = 3),
                    selected = false,
