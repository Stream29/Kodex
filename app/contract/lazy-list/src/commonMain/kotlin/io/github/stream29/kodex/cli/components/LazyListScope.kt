...
internal class LazyListScopeImpl : LazyListScope {
    ...
    fun build(): LazyListIntervalContent =
...
internal class LazyListIntervalContent(
    val intervals: List<LazyListInterval>,
    val itemCount: Int,
)

internal class LazyItemProvider(
    intervalContent: LazyListIntervalContent,
    private val reverseLayout: Boolean,
    nearestRange: IntRange,
) {
    private val intervals = intervalContent.intervals
    val itemCount: Int = intervalContent.itemCount
    private val nearestRangeStart = nearestRange.first.coerceIn(0, itemCount)
    private val nearestRangeEndExclusive = if (nearestRange.isEmpty()) {
        nearestRangeStart
    } else {
        (nearestRange.last.toLong() + 1)
            .coerceIn(nearestRangeStart.toLong(), itemCount.toLong())
            .toInt()
    }
    private val nearestKeys = List(nearestRangeEndExclusive - nearestRangeStart) { offset ->
        calculateKey(nearestRangeStart + offset)
    }
    private val nearestIndicesByKey = HashMap<Any, Int>(nearestKeys.size)

    init {
        nearestKeys.forEachIndexed { offset, key ->
            val index = nearestRangeStart + offset
            require(nearestIndicesByKey.put(key, index) == null) {
    ...
    fun keyAt(index: Int): Any {
        require(index in 0 until itemCount) {
            "Lazy list index $index is out of bounds for $itemCount items."
        }
        return if (index in nearestRangeStart until nearestRangeEndExclusive) {
            nearestKeys[index - nearestRangeStart]
        } else {
            calculateKey(index)
        }
    }

    fun indexOfKey(key: Any): Int? = nearestIndicesByKey[key]
    ...
    private fun calculateKey(index: Int): Any {
        val interval = intervalAt(index)
        return interval.keyFactory.key(index - interval.startIndex, index)
    }
