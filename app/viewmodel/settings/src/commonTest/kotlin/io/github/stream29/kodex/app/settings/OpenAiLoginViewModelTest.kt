    ...
    test("authorization URL is emitted as an effect instead of popup state") {
        coroutineScope {
            ...
            try {
                ...
                assertEquals(
                    OpenAiLoginState.WaitingForAuthorization(effect.attemptId),
    ...
    test("retry reuses the active callback attempt") {
        coroutineScope {
            ...
            try {
                ...
                assertEquals(
                    OpenAiLoginState.BrowserOpenFailed(first.attemptId),
                    viewModel.state.value,
                )
                viewModel.retryBrowser(first.attemptId)
                ...
                assertEquals(
                    OpenAiLoginState.WaitingForAuthorization(first.attemptId),
                    viewModel.state.value,
                )
            } finally {
                viewModel.close()
