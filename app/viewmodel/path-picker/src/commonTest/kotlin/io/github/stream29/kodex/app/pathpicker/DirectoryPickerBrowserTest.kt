...
val directoryPickerBrowserTest by testSuite {
    test("expands current user home shorthand before resolving a directory") {
        ...
        try {
            ...
            val homeListing = browser.load(Path("~")).listing()
            val workspaceListing = browser.load(Path("~", "workspace")).listing()
    ...
    test("resolves a directory and lists only its child directories in name order") {
        ...
        try {
            ...
            val listing = DirectoryPickerBrowser().load(directory).listing()
...
private fun DirectoryPickerBrowserResult.listing(): DirectoryPickerListing =
    assertIs<DirectoryPickerBrowserResult.Success>(this).listing
