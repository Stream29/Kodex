...
public class FileSystemIndexVersioned<T>(
    ...
    override suspend fun revert(untilExclusive: Int) {
        ...
        val pending = Path(directory, ".codex-lite-revert-pending-$operationId")
        val committed = Path(directory, ".codex-lite-revert-committed-$operationId")
    ...
    private suspend fun writeAtomically(destination: Path, content: String) {
        val temporary = Path(directory, ".codex-lite-write-${Uuid.generateV7()}.tmp")
        try {
