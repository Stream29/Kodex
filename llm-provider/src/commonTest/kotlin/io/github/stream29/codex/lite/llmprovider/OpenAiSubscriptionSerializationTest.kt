...
class OpenAiSubscriptionSerializationTest {
    ...
    fun functionToolSerializesExpectedWireShape() {
        val tool = LlmTool.Function(
            ...
            parameters = LlmJsonSchema.objectValue(
                properties = mapOf("foo" to LlmJsonSchema.string()),
            ),
        )
        ...
        val tool = LlmTool.Namespace(
            ...
            tools = listOf(
                LlmNamespaceTool.Function(
                    ...
                    parameters = LlmJsonSchema.objectValue(
                        properties = mapOf("order_id" to LlmJsonSchema.string()),
                    ),
                ),
            ),
        )
    ...
    fun toolSearchToolSerializesExpectedWireShape() {
        val tool = LlmTool.ToolSearch(
            ...
            parameters = LlmJsonSchema.objectValue(
                properties = mapOf("query" to LlmJsonSchema.string("Tool search query")),
                required = listOf("query"),
                additionalProperties = LlmJsonSchemaAdditionalProperties.Boolean(false),
            ),
        )
                ...
                LlmLoadableToolSpec.Namespace(
                    ...
                    tools = listOf(
                        LlmLoadableNamespaceTool.Function(
                            ...
                            deferLoading = true,
                            parameters = LlmJsonSchema.objectValue(
                                properties = mapOf("title" to LlmJsonSchema.string()),
