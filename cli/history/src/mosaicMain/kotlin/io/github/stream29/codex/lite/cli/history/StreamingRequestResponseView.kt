 ...
 *
 * The state-side SharedFlow is the identity of one output item. Re-entering
 * composition starts from an empty local snapshot and replays it exactly once.
 */
@Composable
public fun CodexAgentStateValue.RequestResponse.renderStreamingTail(
    onContentChange: () -> Unit = {},
) {
    when (this) {
        CodexAgentStateValue.RequestResponse.Started -> StreamingStatusTail("Starting response…")
        is CodexAgentStateValue.RequestResponse.Message -> StreamingMessageTail(events, onContentChange)
        is CodexAgentStateValue.RequestResponse.AgentMessage -> StreamingAgentMessageTail(events, onContentChange)
        is CodexAgentStateValue.RequestResponse.Reasoning -> StreamingReasoningTail(events, onContentChange)
        is CodexAgentStateValue.RequestResponse.ToolCall -> StreamingToolCallTail(events, onContentChange)
        is CodexAgentStateValue.RequestResponse.Unknown -> StreamingUnknownTail(events, onContentChange)
    }
}
